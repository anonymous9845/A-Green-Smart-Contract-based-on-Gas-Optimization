...
contract("Contract", async(accounts) => {
    ...
    it("sellArticle", async function() {
        ...
        var result = await market.sellArticle(articleName1, articleDescription1, articlePrice1, {from: seller});
        var {receipt} = result;
    ...
    it("buyArticle", async function() {
        ...
        var articleId = 1;
        var result = await market.buyArticle(articleId, {from: buyer, value: articlePrice1});
        var {receipt} = result;
        var gas = JSON.stringify(receipt.gasUsed);
